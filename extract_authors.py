...
def get_name_data(pers_name):
    ...
    middle_name = combine_all_text(pers_name.find_all('forename',type="middle"))
...
def get_author_metadata(file,authors_list=[],allOrgs=[]):
    ...
    for author in authors:
        ...
        if full_name==None:
            continue
        email=author.email
        author_dict={"name":first_name,"last_name":last_name,"label":full_name,"email":email}
        add_new_author(authors_list,author_dict,seen_names_author)
        orgs_relations = getAffiliations(author,allOrgs,seen_names_orgs,author_dict["author_id"])
...
def get_all_author_metadata(files,allOrgs=[]):
    ...
    for idx,paper in enumerate(files):
        authors_list,relations_org,allOrgs = get_author_metadata(paper,authors_list,allOrgs)
