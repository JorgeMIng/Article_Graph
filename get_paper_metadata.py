...
def get_paper_metadata(file):
    title = extract_element_soup(
        file, "teiHeader.fileDesc.titleStmt.title", None, None).text
    date = extract_element_soup(
        file, "teiHeader.fileDesc.publicationStmt.date", None, None)

    if len(date) == 0:
        date = extract_element_soup(
            file, "teiHeader.fileDesc.sourceDesc.biblStruct.monogr.imprint.date", None, None)
    if len(date) == 0:
        date = None
    ...
    paper_element = {"title": title,
                     "abstract": abstract_text, "release_date": date}
